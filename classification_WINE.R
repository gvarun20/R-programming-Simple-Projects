...
data(wine, package = "rpart")

# Define UI
ui <- fluidPage(
  navbarPage(
    title = "Wine Classification App",
    tabPanel("Explore Data", 
             sidebarLayout(
               sidebarPanel(
                 actionButton("loadData", "Load Wine Dataset")
               ),
               mainPanel(
                 DTOutput("table")
               )
...
server <- function(input, output, session) {
  ...
  # Load wine dataset on button click
  ...
  output$classifyPlot <- renderPlot({
    ...
    classify_model <- randomForest(Type ~ ., data = classify_data())
